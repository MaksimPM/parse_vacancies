...
def main():
    ...
    queries = DBManager(db_name)

    while True:
        query = input('''Выберете действие: 
                    1 - Вывести список всех компаний и количество вакансий у каждой компании.
                    2 - Вывести список всех вакансий с указанием названия компании, названия вакансии, зарплаты 
        ...
        if query == '1':
            queries.get_companies_and_vacancies_count()
        elif query == '2':
            queries.get_all_vacancies()
        elif query == '3':
            queries.get_avg_salary()
        elif query == '4':
            queries.get_vacancies_with_higher_salary()
        elif query == '5':
            keyword = input('Введите ключевое слово: ')
            queries.get_vacancies_with_keyword(keyword)
        elif query == '0':
            print('До свидания!')
            new_bd.cur_close()
            new_bd.conn_close()
